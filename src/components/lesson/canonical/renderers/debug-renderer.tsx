...
import { Bug, RotateCcw, Play, Terminal, CheckCircle2, XCircle, AlertTriangle } from "lucide-react";
...
export function DebugRenderer({
  ...
  const { buggyCode, bugDescription, language, hints, testCases, fixRequirements } =
    activity.content;
  ...
  useEffect(() => {
    if (state.response === undefined || state.response === null) {
      onResponse(buggyCode);
    }
  }, [buggyCode, state.response, onResponse]);
    ...
    <ActivityContainer id={`activity-${activity.id}`}>
      ...
      <div className="p-6 md:p-8 flex flex-col gap-6">
        <div className="space-y-3">
          ...
          <div className="flex items-start gap-3 p-4 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-950 dark:text-rose-200 text-sm">
            <AlertTriangle className="w-5 h-5 text-rose-500 shrink-0 mt-0.5" />
            <div>
              <p className="font-semibold text-xs text-rose-600 dark:text-rose-400 uppercase tracking-wider mb-0.5">
                Reported Bug
              </p>
              <p>{bugDescription}</p>
            </div>
        ...
        <div className="flex items-center gap-3">
          <Button size="sm" onClick={handleRunTest} disabled={isRunning} className="gap-2 h-9">
        ...
        {testResults.length > 0 && (
          <div className="space-y-2">
            <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
              Verification Tests ({testResults.filter((t) => t.passed).length}/{testResults.length})
            </h3>
