...
export function MasteryEnginePage() {
  ...
  const handleOpenRefresher = (topic: TopicMasteryRecord) => {
    setSelectedRefresherTopic(topic);
    setRefresherModalOpen(true);
  };
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      {/* AI Refresher Dialog */}
      <Dialog open={refresherModalOpen} onOpenChange={setRefresherModalOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Sparkles className="h-5 w-5 text-amber-500" /> AI Concept Refresher
            </DialogTitle>
          </DialogHeader>
          {selectedRefresherTopic && (
            <div className="space-y-4 pt-3">
              <div>
                <Badge variant="secondary" className="mb-1">
                  {selectedRefresherTopic.category}
                </Badge>
                <h3 className="text-lg font-bold">{selectedRefresherTopic.topicTitle}</h3>
              </div>

              <div className="rounded-xl p-4 bg-muted/40 border border-border/40 text-xs leading-relaxed space-y-3">
                <p className="font-medium text-foreground">💡 Core Mental Model Refresher:</p>
                <p className="text-muted-foreground">
                  {selectedRefresherTopic.category === "JavaScript" &&
                    "JavaScript concepts center around execution context, the call stack, lexical scope, and closures. Ensure you master the event loop, microtask prioritization, and prototype chains."}
                  {selectedRefresherTopic.category === "React" &&
                    "React 19 centers around automatic batching, server actions, concurrent rendering APIs (useTransition, useDeferredValue), and fiber architecture optimization."}
                  {selectedRefresherTopic.category === "CSS" &&
                    "CSS Layout focuses on modern layouts like Flexbox and CSS Grid. Always consider subgrid features, layout containment, structural stacking contexts, and fluid container sizing."}
                  {selectedRefresherTopic.category === "TypeScript" &&
                    "TypeScript mastery means leveraging advanced types (conditional types, mapped types, template literal types), utility types, and strict parameter modeling."}
                  {selectedRefresherTopic.category === "Performance" &&
                    "Performance optimization covers image optimization, dynamic imports, route-based splitting, CSS containment, debounced inputs, and minimizing paint reflows."}
                  {selectedRefresherTopic.category === "System Design" &&
                    "System Design highlights decoupled component modularity, micro-frontends, state synchronization across domains, offline capabilities, and design token integration."}
                  {![
                    "JavaScript",
                    "React",
                    "CSS",
                    ...
                    "Performance",
                    "System Design",
                  ].includes(selectedRefresherTopic.category) &&
                    "Mastering this concept involves structured pattern decomposition, edge-case testing, performance tracing, and keeping state localized and clean."}
                </p>
                <div className="pt-2 border-t flex flex-col gap-1.5 text-[11px] text-muted-foreground">
                  <span>
                    📊 Current Confidence Level:{" "}
                    <strong className="text-primary">{selectedRefresherTopic.confidence}%</strong>
                  </span>
                  <span>
                    🔄 Total Reviews Performed:{" "}
                    <strong>{selectedRefresherTopic.reviewCount} reviews</strong>
                  </span>
                  <span>
                    🕒 Next review scheduled in{" "}
                    <strong>{selectedRefresherTopic.intervalDays} days</strong>
                  </span>
                </div>
              </div>

              <div className="flex gap-2">
                <Link to="/mentor" className="flex-1">
                  <Button className="w-full gap-1.5 text-xs font-semibold" size="sm">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setRefresherModalOpen(false)}
                  className="text-xs"
                >
                  Got it
                </Button>
