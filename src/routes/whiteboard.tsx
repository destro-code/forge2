...
import type { WhiteboardSnapshot } from "@/lib/types";
...
export function WhiteboardPage() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const snap = saveWhiteboardSnapshot({
        id: snapshotId || undefined,
        title: currentTitle,
        presetId: selectedPreset?.id,
        elements: { code, activeMode, nodes, edges },
      });
      if (!snapshotId) {
  ...
  }, [code, activeMode, nodes, edges, saveWhiteboardSnapshot, selectedPreset, currentTitle]);
  ...
  const handleNewWhiteboard = () => {
    setSnapshotId("");
    setCurrentTitle("New Whiteboard");
    setCode("");
    ...
    toast.success("Created new whiteboard");
  };

  const loadSnapshot = (snap: WhiteboardSnapshot) => {
    setSnapshotId(snap.id);
    const elems = snap.elements as Record<string, unknown>;
    if (elems) {
      if (elems.code) setCode(elems.code as string);
      if (elems.activeMode) setActiveMode(elems.activeMode as Mode);
      if (elems.nodes) setNodes(elems.nodes as ArchNode[]);
      if (elems.edges) setEdges(elems.edges as ArchEdge[]);
    }
    const preset = WHITEBOARD_PRESETS.find((p) => p.id === snap.presetId);
  ...
  const buildSVG = () => {
    let svg = `<svg xmlns="http://www.w3.org/2000/svg" width="800" height="600" style="background-color: #0f172a;">`;

    edges.forEach((edge) => {
      const from = nodes.find((n) => n.id === edge.from);
      const to = nodes.find((n) => n.id === edge.to);
      if (from && to) {
        svg += `<line x1="${from.x + 50}" y1="${from.y + 25}" x2="${to.x + 50}" y2="${to.y + 25}" stroke="#475569" stroke-width="2" />`;
        svg += `<text x="${(from.x + to.x) / 2}" y="${(from.y + to.y) / 2 - 5}" fill="#94a3b8" font-size="10" font-family="sans-serif" text-anchor="middle">${edge.label}</text>`;
      }
    });

    nodes.forEach((node) => {
      const color = node.type === "client" ? "#0f172a" : node.type === "db" ? "#7f1d1d" : "#1e293b";
      const stroke =
        node.type === "client" ? "#cbd5e1" : node.type === "db" ? "#f87171" : "#475569";
      svg += `<rect x="${node.x}" y="${node.y}" width="100" height="50" rx="8" fill="${color}" stroke="${stroke}" stroke-width="2" />`;
      svg += `<text x="${node.x + 50}" y="${node.y + 30}" fill="#f8fafc" font-size="11" font-family="sans-serif" text-anchor="middle">${node.label}</text>`;
    });
  ...
  const handleExportSVG = () => {
    ...
    toast.success("Exported Architecture as SVG");
  };
  ...
  const handleExportPNG = () => {
    ...
    img.onload = () => {
      ...
      URL.revokeObjectURL(url);
      toast.success("Exported Architecture as PNG");
    };
    img.src = url;
  };
      ...
      <header className="flex shrink-0 items-center justify-between border-b px-4 py-2.5">
        ...
        <div className="flex items-center gap-2 mr-auto ml-4">
          <Input
            ...
            className="h-8 text-xs font-semibold bg-transparent border-transparent hover:border-border focus:border-primary focus:ring-1 w-48"
          />
        </div>
        ...
        <Dialog open={snapshotsDialogOpen} onOpenChange={setSnapshotsDialogOpen}>
          <DialogTrigger asChild>
            <Button variant="outline" size="sm" className="h-8 gap-1.5 text-xs mr-2">
              <FolderOpen className="h-3.5 w-3.5" />
              Snapshots ({(whiteboardSnapshots || []).length})
            </Button>
          </DialogTrigger>
          <DialogContent className="max-w-2xl">
            <DialogHeader className="flex flex-row items-center justify-between">
              ...
              <Button size="sm" onClick={handleNewWhiteboard} className="gap-1.5 h-8">
                <Plus className="h-3.5 w-3.5" /> New Whiteboard
              </Button>
            </DialogHeader>
            <div className="grid gap-3 pt-2 max-h-[60vh] overflow-y-auto">
              {(whiteboardSnapshots || []).length === 0 ? (
                <div className="text-center text-sm text-muted-foreground py-8">
                  No snapshots saved yet. Your work will auto-save here.
                </div>
              ) : (
                (whiteboardSnapshots || []).map((snap) => (
                  ...
                  >
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-muted-foreground font-mono">ID: {snap.id}</p>
                    </div>
                    <Button
                      size="xs"
                      variant="ghost"
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      className="h-7 gap-1 ml-4"
                    >
                      <ImageIcon className="h-3 w-3" /> Export SVG
                    </Button>
                    <Button
                      size="xs"
                      variant="outline"
                      ...
                      className="h-7 gap-1"
                    >
                      <ImageIcon className="h-3 w-3" /> Export PNG
                    </Button>
                    <Button size="xs" onClick={handleAddNode} className="h-7 gap-1 ml-4">
