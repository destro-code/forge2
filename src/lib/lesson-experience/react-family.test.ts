...
describe("React component browser family", () => {
  it("renders JSX and captures bounded evidence", async () => {
    const result = await runReactComponent({
      source:
  ...
  it("captures runtime errors and rejects stale revisions", async () => {
    const result = await runReactComponent({
      source: "function App() { throw new Error('boom'); }",
    });
