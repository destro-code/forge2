...
function runtimeUrl(nonce: string, runId: string, runRevision: number) { const query = new URLSearchParams({ nonce, runId, revision: String(runRevision) }); return `/react-runtime.html?${query.toString()}`; }
