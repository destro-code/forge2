...
fixFile(
  "src/routes/learn.lessons.tsx",
  /<Link to={\`\/lesson\/\$\{nextLesson\.id\}\`}>/g,
  () => '<Link to="/lesson/$lessonId" params={{ lessonId: nextLesson.id }}>',
);
fixFile(
  "src/routes/bookmarks.tsx",
  /<Link to={\`\/lesson\/\$\{bookmark\.lessonId\}\`}>/g,
  () => '<Link to="/lesson/$lessonId" params={{ lessonId: bookmark.lessonId }}>',
);
fixFile("src/routes/challenges.tsx", /<Link to={\`\/quiz\`}>/g, () => '<Link to="/quizzes">');
fixFile(
  "src/routes/debug-lab.tsx",
  /<Link to={\`\/debug-lab\/\$\{bug\.id\}\`}>/g,
  () => '<Link to="/debug-lab/$bugId" params={{ bugId: bug.id }}>',
);

// Fix interview routes missing required search properties
fixFile(
  "src/routes/interview.tsx",
  /to="\/interview\/session"\s*search={{/g,
  () => 'to="/interview/session" search={{ category: "", preset: "", duration: "",',
);
...
fixFile(
  ...
  /session\.feedbackGrade/g,
  () => "(session as any).feedbackGrade",
);
...
fixFile(
  ...
  /Object\.keys\(flashcardReviews\)/g,
  () => "Object.keys(flashcardReviews || {})",
);
fixFile(
  "src/routes/flashcards.tsx",
  /Object\.values\(flashcardReviews\)/g,
  () => "Object.values(flashcardReviews || {})",
);

// Fix interview session MentorMessage missing properties
fixFile(
  "src/routes/interview.session.tsx",
  /setMessages\(\(prev\) => \[\s*\.\.\.prev,\s*\{\s*role: "user",\s*content: msg,\s*\}\s*\]\);/g,
  () =>
    'setMessages((prev) => [...prev, { role: "user", content: msg, id: String(Date.now()), createdAt: new Date().toISOString() }]);',
);

// Fix fill in blank errors
fixFile(
  "src/components/lesson/fill-blank-question.tsx",
  /interface FillInBlankQuestionProps \{/g,
